...
def sort_assign(idxs, labels, num_users, num_imgs, idx_shard, dict_users):
    ...
    for i in range(num_users):
        ...
        # TODO modify sharding from fix 2x300
        # measuring the label distribution amongst parties
        #division = np.zeros([num_users, len(set(labels))])
        #for i in range(num_users):
        #    for x in dict_users[i]:
        #        division[i][labels[int(x)]] = division[i][labels[int(x)]] + 1
    return dict_users
